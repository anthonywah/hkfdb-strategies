...
def backtest(para_combination):
    ...
    nw_len                 = para_combination['nw_len']
    nw_zscore_threshold    = para_combination['nw_zscore_threshold']
    nw_zscore_sign         = para_combination['nw_zscore_sign']
    zscore_cross_direction = para_combination['zscore_cross_direction']
    ...
    ##### stra specific #####
    df['nw_avg'] = df['nw_chg'].rolling(nw_len).mean()
    df['nw_std'] = df['nw_chg'].rolling(nw_len).std()
    df['nw_zscore'] = (df['nw_chg'] - df['nw_avg']) / df['nw_std']
    df['nw_zscore-1'] = df['nw_zscore'].shift(1)

    if nw_zscore_sign == 'negative': nw_zscore_threshold = -1 * nw_zscore_threshold

    if zscore_cross_direction == 'cross_over':
        df['trade_logic'] = (df['nw_zscore'] > nw_zscore_threshold) & (df['nw_zscore-1'] <= nw_zscore_threshold)
    elif zscore_cross_direction == 'cross_under':
        df['trade_logic'] = (df['nw_zscore'] < nw_zscore_threshold) & (df['nw_zscore-1'] >= nw_zscore_threshold)
...
def get_secondary_data(para_dict, code_list, df_dict, start_date, end_date):
    start_year = start_date[0:4]
    end_year   = end_date[0:4]
    year_diff = int(end_year) - int(start_year)

    ccass_id_dict = {'A00003':[], 'A00004':[]}

    for ccass_id in ccass_id_dict:
        ...
        for i in range(year_diff + 1):
            ...
            file_path = os.path.join(secondary_data_folder, ccass_id + '_' + str(dl_start_date) + '_' + str(dl_end_date) + '.parquet')

            try:

                if os.path.isfile(file_path) and not update_data:
                    df2 = pd.read_parquet(file_path)
                    print(datetime.datetime.now(), ' successfully read secondary data', ccass_id, dl_start_date)
                else:
                    df2 = client.get_ccass_by_id(ccass_id, dl_start_date, dl_end_date)
                    time.sleep(1)
                    df2.to_parquet(file_path)
                    print(datetime.datetime.now(), ' successfully get secondary data', ccass_id, dl_start_date)
                ccass_id_dict[ccass_id].append(df2)
            except:
                print(datetime.datetime.now(), ' error in  getting secondary data', ccass_id, dl_start_date)
                time.sleep(1)

    for code, df in df_dict.items():
        df_sh = pd.concat(ccass_id_dict['A00003'])
        df_sz = pd.concat(ccass_id_dict['A00004'])

        df_sh = df_sh[df_sh['code'] == code]
        df_sz = df_sz[df_sz['code'] == code]

        df_sh = df_sh.rename(columns={'percent':'percent_sh'})
        df_sz = df_sz.rename(columns={'percent':'percent_sz'})

        if len(df_sh) + len(df_sz) == 0:
            code_list.remove(code)
        else:
            df2 = pd.concat([df_sh, df_sz], axis=1)
            df2['nw'] = df2['percent_sh'] + df2['percent_sz']


            df2['nw_chg'] = 100 * df2['nw'].pct_change()
            df2 = df2[['nw_chg']].round(3)
            ...
            df['nw_chg'] = df['nw_chg'].shift(1)

            df_dict[code] = df

    para_dict['code'] = code_list
...
if __name__ == '__main__':
    ...
    para_dict = {
        ...
        'holding_day'           : [5, 10, 20],
        'nw_len'                : [50, 100, 250],
        'nw_zscore_threshold'   : [1, 1.5, 2, 2.5, 3],
        'nw_zscore_sign'        : ['positive', 'negative'],
        'zscore_cross_direction' : ['cross_over', 'cross_under']
    ...
    df_dict, para_dict = get_secondary_data(para_dict, code_list, df_dict, start_date, end_date)
